...
def _build_review_prompt(body: str) -> str:
    plain = re.sub(r'<[^>]+>', '', body)
    return f"""아래 블로그 본문을 전체 글 단위로 평가해줘.

## PASS 조건 (하나라도 해당하면 PASS)
- 구체적 수치(숫자, 금액, 날짜, 퍼센트)가 2개 이상 있다
- 글 전반에 정보 밀도가 있다 (핵심 사실이 3개 이상 담겨 있다)
- 독자가 읽고 나서 뭔가 하나라도 알게 되는 글이다

## FAIL 조건 (모두 해당할 때만 FAIL)
아래 조건이 **동시에 모두** 충족될 때만 REVIEW_RESULT: FAIL을 반환해:
1. "업계 관계자는", "업계에서는", "보도됐다", "알려졌다" 같은 **출처 없는 보도체 문장**이 2개 이상
2. 구체적 수치가 **전혀 없거나** 1개 이하
3. 글의 절반 이상이 전환 문장·추상 감상·뻔한 요약으로만 채워져 있다

## 중요
- 전환 문장(예: "이 차이는 크다", "달라졌다"), 수사적 질문(예: "왜일까?"), 요약 문장은 **자연스러운 한국어 글쓰기**이므로 그 자체로 FAIL 사유가 되지 않는다.
- 단순히 "뻔한 표현이 있다"만으로 FAIL 주지 마라. 전체 정보량과 독자 가치를 기준으로 판단해라.
- 위 FAIL 조건을 모두 만족하는 경우에만 FAIL을 반환하고, 그 외에는 모두 PASS다.

개선이 필요한 문장이 있으면 FAILED_SENTENCES에 표시해줘 (PASS여도 표시 가능).
...
def _parse_review(result: str) -> tuple[bool, str]:
    ...
    # 실패 문장 7개 이하 → 수정 피드백만 전달, PASS 처리 (전환·요약 문장은 자연스러운 한국어 글쓰기)
    if len(failed_lines) <= 7:
        return True, ''
